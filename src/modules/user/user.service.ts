...
export class UserService {
    ...
    async delete(id: number): Promise<void>{
      const userExixist = await this._userRepository.findOne(id,{where: {status: status.ACTIVE}});
      if(!userExixist){
        throw new NotFoundException();
      }
    ...
    async setRoleToUser(userId:number,roleId:number){
      ...
      if(!userExist){
        throw new NotFoundException('user does no exist');
      }
      
      const roleExist = await this._roleRepository.findOne(roleId,{where: {status: status.ACTIVE}});
      if(!userExist){
        throw new NotFoundException('Role does no exist');
      }
